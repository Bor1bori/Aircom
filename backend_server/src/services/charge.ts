...
/**
 * @description 구독 메뉴 가져오기
 */
export const getSubscriptionMenus = async () => {
  return await SubscriptionMenu.findAll();
};
...
export const getActiveSubscription = async (user: User) => {
  ...
  return {
    ...
    subscription: await activeSubscribe.getSubscriptionMenu()
  };
};
